...
class Game extends Component {
	...
	axiosFetchData = () => {
		...
		axios({
			...
			data: `fields slug, platforms.*, rating, cover.*, name, release_dates.*, url, videos.*, websites.*, alternative_names.*, aggregated_rating_count, total_rating, age_ratings.*, artworks.*, dlcs.*, expansions.*, game_modes.*, genres.*, multiplayer_modes.*, popularity, screenshots.*, status, storyline, summary, themes.*, time_to_beat, similar_games.cover.*, similar_games.name, similar_games.platforms.*, similar_games.slug, similar_games.rating, similar_games.release_dates.*, *; where slug = "${this
				.state.gameSlug}";`
		})
			.then((response) => {
	...
	firestoreGetData = async () => {
		...
		this.firestoreGamesGetFollowers();
	...
	followHandler = () => {
		const db = fire.firestore();
		const gamesRef = db.collection('games').doc(this.state.gameSlug);
		const userRef = db.collection('users').doc(this.props.contextprops.user.uid);
		...
		if (!followArray.includes(this.props.contextprops.user.uid)) {
			userRef
				.update({
					gamesFollowed: firebase.firestore.FieldValue.arrayUnion(this.state.gameSlug)
				})
				.then(() => this.firestoreGamesGetFollowers());
		} else {
			userRef
				.update({
					gamesFollowed: firebase.firestore.FieldValue.arrayRemove(this.state.gameSlug)
				})
				.then(() => this.firestoreGamesGetFollowers());
		}
	};
	...
	render() {
		...
		if (loading) {
			...
			const backgroundUrl = games.hasOwnProperty('screenshots')
				? {
						backgroundImage: `url(https://images.igdb.com/igdb/image/upload/t_1080p/${games.screenshots[0]
							.image_id}.jpg)`
					}
				: { background: 'grey' };

			const cover = games.hasOwnProperty('cover')
				? `https://images.igdb.com/igdb/image/upload/t_cover_big/${games.cover.image_id}.jpg`
				: undefined;
			...
			const followLabel = this.props.contextprops.user !== null && this.state.followed ? 'Following' : 'Follow';
			...
			return (
							...
							<div className="content">
								...
								<div className="genresContainer">
									{games.genres.map((genres) => (
										<Link to={`/browse/genres = ${genres.id}`} className="genres">
											{genres.name}
										</Link>
									))}
								</div>
										...
										{games.hasOwnProperty('game_modes') ? (
											<React.Fragment>
												...
												<div style={{ textAlign: 'right', height: '100%' }}>
													{games.game_modes.map((modes) => <div>{modes.name}</div>)}
												</div>
											</React.Fragment>
										) : (
											undefined
                ...
                <div style={{ textAlign: "right" }}>test</div> */}
								...
								<div className="gameContent">
									...
									{currentTab === 'Similar Games' && <SimilarGames gameData={gameData} />}
								</div>
